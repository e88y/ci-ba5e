...
require_once (APPPATH.'core/BASE_Permissions.php');
...
class BASE_Controller extends CI_Controller
{
	 ...
	 * The 'locale_code' is the array-key
	 * @var array
	 */
	 ...
	 */
	function __construct($requires_login=false, $requires_root=false)
	{	
		//$backtrace = debug_backtrace();$backtrace[1]['function']
		parent::__construct();
		...
		write2Debugfile(self::DEBUG_FILENAME, "\nBASE_Controller::Class[".$this->router->class."] method[".$this->router->method."] login-required [".$this->requires_login."] root-required[".$this->requires_root."] time [".time()."] Environment [".ENVIRONMENT."]", false);
		write2Debugfile(self::DEBUG_FILENAME, "\nsubdomain [$this->subdomain]", true);
		...
		$this->config->set_item('language', $locale_folder);
		...
		$this->title 		= ucfirst(lang( strtolower(get_called_class()) ))." | ".$this->config->item('site_title');
		$this->description 	= $this->config->item('site_description');
		$this->keywords 	= $this->config->item('site_keywords').", ".$this->pageName;
		...
		self::checkPermissions();		// when the user has isufficient permissions, we render an error page (which prevents the execution of the controller method) 
	...
	protected function render($view, $rendermode="FULLPAGE")
	{
		...
		if ($rendermode == E_RENDERMODE::FULLPAGE){
			self::setSessionItem(E_SESSION_ITEM::LAST_URL, $this->uri->uri_string);
		}
		self::setViewData(E_RESEREVED_DATAKEYS::CLIENT_ID, $this->client_id);
		self::setViewData(E_RESEREVED_DATAKEYS::JS_ENABLED, $this->getSessionData(E_SESSION_ITEM::JS_ENABLED));
        ...
        switch (strtoupper($rendermode))
        {
        	...
        	case E_RENDERMODE::AJAX:		// echo a json encoded BASE_Result containing the view in the data-attribute
        		...
        		$response = new BASE_Result($this->load->view($view, $this->data, true), $this->data["error"], $this->data["extra"], $this->data["status"], $this->data["redirect"] );
        		write2Debugfile(self::DEBUG_FILENAME, "AJAX\n".json_encode($response) );
        		echo json_encode($response);			
        		break;
        	
        	case E_RENDERMODE::AJAX_PLAIN:	// load/echo the view
        		write2Debugfile(self::DEBUG_FILENAME, "AJAX_PLAIN\n".$this->load->view($view, $this->data, true) );
        	...
        	case E_RENDERMODE::FULLPAGE : // echo complete page
        			...
        			self::setViewData(E_RESEREVED_DATAKEYS::DEBUG_FILENAME, ( $callerClass::DEBUG_FILENAME !== null ? $callerClass::DEBUG_FILENAME : "debug.log"));
					self::setViewData(E_RESEREVED_DATAKEYS::CALLER_CLASS, $callerClass);
        	...
        	default : 
				...
				if($this->hasNav)
				{
					...
					$toMenu["error"]				= $this->data["error"];
					$toMenu["data"]					= $this->data["data"];
					$toMenu["extra"]				= $this->data["extra"];
					$toMenu["status"]				= $this->data["status"];
					
					write2Debugfile(self::DEBUG_FILENAME, " - toMenu:".print_r($toMenu, true));
					// ..:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::..
					$this->load->helper("menu"); 
					$toBody["menu"] = $this->load->view("template/menu", $toMenu, true);
				}
				...
				if ($this->hasSidebar)
				{
					...
					$toSidebar["sidebar_selected_item"]	= get_cookie(E_SESSION_ITEM::SIDEBAR_SELECTED_ITEM);
					
					write2Debugfile(self::DEBUG_FILENAME, " - toSidebar:".print_r($toSidebar, true));
					// ..:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::..
					$toBody["sidebar"] 					= $this->load->view("template/sidebar", $toSidebar, true);
				}
				...
				{	
					...
					// ..:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::..
					$toBody["breadcrump"] 		= $this->load->view("template/breadcrump", $toBreadcrump, true);
				}
				...
				$toTpl["javascriptCalls"]	= $this->javascriptCalls;
				$toTpl["css"]				= $this->css;
				$toTpl["fonts"] 			= $this->fonts;
				
				$toTpl["theme"] 			= $this->theme;
				$toTpl["loaded_language"] 	= $this->loaded_language;
				$toTpl["plugins_css"]		= array_unique($this->plugin_css);
				$toTpl["plugins_js"]		= array_unique($this->plugin_js);
				$toTpl["body"] 				= $this->load->view("template/main", $toBody, true);
		...
		$elapsedTime = $this->benchmark->elapsed_time('render_start', 'render_end');
	...
	private function profiler($rendermode)
	{
		if ($rendermode == E_RENDERMODE::FULLPAGE)
		{
			if ($this->config->item('enable_profiler') == 1 && ENVIRONMENT != E_ENVIRONMENT::PRODUCTION){
	...
	{
		$rendermode 			= $this->input->post("rendermode") != "" ? $this->input->post("rendermode") : E_RENDERMODE::FULLPAGE;
	...
	public static function hasPermission($permission)
	{
		...
		{
			$permissions 	= $_SESSION[E_SESSION_ITEM::USER_PERMISSIONS];
	...
	{
		$rendermode 		= $this->input->post("rendermode") != "" ? $this->input->post("rendermode") : E_RENDERMODE::FULLPAGE;
		$grant_without_rule = false;
		$public_controllers = array('home', 'register', 'modal', 'login', 'logout'); // these controllers do not require immediate login credentials or permissions
		...
		{
			$result = new BASE_Result(null, lang("msg_you_dont_have_the_permission_to_access_this_method"));
			self::setData($result);
	...
	private function addDefaultIncludes()
	{
		array_push($this->plugin_js,
			E_PLUGINS_JS::JQUERY,
			E_PLUGINS_JS::JQUERY_UI,	// we cannot allow to add this lib via 'addPlugins' until we have a solution for the correct inclusion order
			E_PLUGINS_JS::BOOTSTRAP,
			//E_PLUGINS_JS::JQUERY_UI_TOUCHPUNCH,	// touch support
			E_PLUGINS_JS::JQUERY_NICESCROLL,
			E_PLUGINS_JS::JQUERY_BLOCKUI,
			E_PLUGINS_JS::JQUERY_GROWL,
			E_PLUGINS_JS::MOMENT_JS_WITH_LOCALES,
			E_PLUGINS_JS::AUTONUMERIC);
	...
	public function display_error($error_view)
	{
		if (E_ERROR_VIEW::isValidValue($error_view)){
			...
			$this->load->view($error_view, $this->data, true);
		}
		else{
