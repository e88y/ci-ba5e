...
class Datatables
{
	...
	*/
	...
	private function get_filtering()
	{
		...
		//write2Debugfile("Datatables.log", "get_filtering:\ncolumns:".print_r($mColArray, true)."\n\nsearch:".print_r($search, true));
		...
		$sWhere = '';
		for($i = 0; $i < count((array)$mColArray); $i++)
		{
	...
	private function produce_output($output, $charset)
	{
		...
		{
			$row_val 			= html_escape($row_val); // ADDED 03.12.2018:meb >> avoid XSS output
		...
		if($output == 'json')
		{
			...
			(
				...
				'data'                => $aaData,
				'user_settings'		  => ""//$this->load_user_settings()
			);
	...
	private function get_total_results($filtering = false)
	{
		...
		write2Debugfile("Datatables.log", "get:\n ".$this->ci->db->last_query());
	...
	/**
	 * load user defined table settings
	 */
	private function load_user_settings()
	{
		$client_id 	= $this->ci->client_id;
		$user_id 	= $this->ci->user_id;
		$table_id	= $this->ci->input->post('table_id');

		if ($client_id != "" && $user_id != "" && $table_id != "")
		{
			$this->ci->db
				->select('*')
				->from( TBL_USER_TABLE_CONFIGURATION )
